...
<main>
    <h1 class="uk-margin-left">Закладки</h1>
    <?php
      if ($projectList) :?>
        ...
        <div class="uk-card uk-card-default project-item">
            ...
            <div class="uk-card-footer likes project-card__footer" >
                <button class="like uk-button <?php if(Project::checkLike($projectItem['id'], $_SESSION['user']) == true) echo "active_h";?>" data-id="<?php echo $projectItem['id']; ?>">
                    <span class="likes-count"><a uk-icon="heart"></a><?php echo $count = Project::likesCount($projectItem['id']); ?></span>
                </button>
                <span><span uk-icon="comments" class="comment-icon"></span><?php echo $count = Project::commentsCount($projectItem['id']); ?></span>
            </div>
            <?php else :?>
            <div class="uk-card-footer likes project-card__footer" >
                <span><span uk-icon="heart" class="heart-icon"></span><?php echo $count = Project::likesCount($projectItem['id']); ?></span>
                <span><span uk-icon="comments" class="comment-icon"></span><?php echo $count = Project::commentsCount($projectItem['id']); ?></span>
            </div>
            <?php endif; ?>
        </div>
